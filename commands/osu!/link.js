...
module.exports = {
	...
	async execute(message, args) {
		...
		try {
			...
			return message.channel.send(`Linked ${message.author} to ${args.join(' ').replace(/[^\w\s]/gi, '')}`);
		} catch(e) {
			if (e.name === 'SequelizeUniqueConstraintError') {
				try {
					const upUser = await Users.update({
						user_osu: args.join(' ').replace(/[^\w\s]/gi, ''),
					},
					{
						where: { user_id: message.author.id },
					});
					if (upUser > 0) {
						return message.reply(`Updated link to ${args.join(' ').replace(/[^\w\s]/gi, '')}`);
					}
				} catch(err) {
					console.error(err);
